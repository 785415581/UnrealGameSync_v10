...
#define USECOUSTOMFUNCTION
#define COUSTOMFUNCTION
using System;
	...
	partial class WorkspaceControl : UserControl, IMainWindowTabPanel
	{
		...
		void StartWorkspaceUpdate(int ChangeNumber, WorkspaceUpdateOptions Options, WorkspaceUpdateCallback Callback)
		{
			...
			if (Options.HasFlag(WorkspaceUpdateOptions.SyncArchives))
			{
				...
				{
					//Console.WriteLine("Archive is {0}", Archive.BasePath);
#if USECOUSTOMFUNCTION
					string ArchivePath = GetArchiveKeyForChangeNumber(Archive, ChangeNumber, true);
#else
		...
		void UpdateComplete(WorkspaceUpdateContext Context, WorkspaceUpdateResult Result, string ResultMessage)
		{
			...
			else if (Result == WorkspaceUpdateResult.Success)
			{
				...
				{
					LaunchEditor();
					//Console.WriteLine("1179 UpdateComplete");
				}
...
#if COUSTOMFUNCTION
		// custom function
		private string GetArchiveKeyForChangeNumber(IArchiveInfo Archive, int ChangeNumber, bool defination)
		{
			string output;
			string ArchivePath;
			if (defination)
			{
				...
				string NoRevsionArchive = Archive.BasePath;
				string ServerAndPort = Workspace.Perforce.ServerAndPort;
				string UserName = Workspace.Perforce.UserName;
				string ClientName = Workspace.Perforce.ClientName;
				...
				BasicCommandArgs.Append(" filelog ");
				BasicCommandArgs.Append(Archive.BasePath);
				string FullCommandLine = BasicCommandArgs.ToString();
				//string CommandLine = " filelog ";
				//BasicCommandArgs += Archive.BasePath;
				//string ArchivePath = "//ProjectX_Binary/main/++P4Test+dev_personal_qinjiaxin-ProjectTestEditor.zip#8";

				RunCMDCommand(FullCommandLine, out output);
				Dictionary<int, int> CollectValue = new Dictionary<int, int>();
				foreach (string line in output.Split('.'))
				{
					if (line.Contains('#'))
					{
						...
						string SRevision;
						string SCLNumber;
						Regex RevisionRegex = new Regex(@"#\d+");
						SRevision = RevisionRegex.Match(line).Groups[0].Value;

						Regex CLNumberRegex = new Regex(@"CL\s\d+");
						SCLNumber = CLNumberRegex.Match(line).Groups[0].Value;

						Revision = int.Parse(System.Text.RegularExpressions.Regex.Replace(SRevision, @"[^0-9]+", ""));
						CLNumber = int.Parse(System.Text.RegularExpressions.Regex.Replace(SCLNumber, @"[^0-9]+", ""));
						if (CLNumber < ChangeNumber)
						{
							CollectValue.Add(Revision, CLNumber);
						}
					}
				}

				Dictionary<int, int> dic1_SortedByKey = CollectValue.OrderByDescending(p => p.Key).ToDictionary(p => p.Key, o => o.Value);

				if (dic1_SortedByKey.Count > 0)
				{
					int re;
					int CL;
					re = dic1_SortedByKey.First().Key;
					CL = dic1_SortedByKey.First().Value;
					ArchivePath = String.Format("{0}#{1}", NoRevsionArchive, re);
				}
				else
				{
					ArchivePath = GetArchiveKeyForChangeNumber(Archive, ChangeNumber);
				}
			}
			else
			{
		...
		public string GetLastValidBianry(string ArchiveBasePath, int ChangeNumber)
		{
			string output;
			int re;
			int CL;
			...
			BasicCommandArgs.Append(" filelog ");
			BasicCommandArgs.Append(ArchiveBasePath);
			string FullCommandLine = BasicCommandArgs.ToString();
			//string CommandLine = " filelog ";
			//BasicCommandArgs += Archive.BasePath;
			//string ArchivePath = "//ProjectX_Binary/main/++P4Test+dev_personal_qinjiaxin-ProjectTestEditor.zip#8";
		...
		public static void RunCMDCommand(string CommandLine, out string OutPut)
		{
...
#if USECOUSTOMFUNCTION
			PerforceChangeDetails Details;
			PerforceMonitor.TryGetChangeDetails(ChangeNumber, out Details);
			if (Details != null)
			{
				...
				else if (Details.bContainsContent)
				{
					return true;
				}
		...
		private void BuildList_DrawSubItem(object sender, DrawListViewSubItemEventArgs e)
		{
			...
			else if (e.ColumnIndex == DescriptionColumn.Index)
			{
				...
				//Console.WriteLine("{0} is description {1}, have {2} DescriptionBadges", Change.Number, e.SubItem.Text, Layout.DescriptionBadges.Count);
			...
			//else if (e.ColumnIndex == NewestBianry.Index)
			//{
			//	if (e.ItemIndex < 1)
			//	{
			...
			//			string CL = GetLastValidBianry(Archive.BasePath, Change.Number);
			//			TextRenderer.DrawText(e.Graphics, CL, CurrentFont, e.Bounds, TextColor, TextFormatFlags.Left | TextFormatFlags.VerticalCenter | TextFormatFlags.EndEllipsis | TextFormatFlags.NoPrefix);
			//		}
			//	}

			//	//TextRenderer.DrawText(e.Graphics, Change.Number.ToString(), CurrentFont, e.Bounds, TextColor, TextFormatFlags.Left | TextFormatFlags.VerticalCenter | TextFormatFlags.EndEllipsis | TextFormatFlags.NoPrefix);
			//}
					...
					foreach (string BadgeDefinition in BadgeDefinitions.Distinct())
					{
						...
						if (!String.IsNullOrEmpty(Name) && !String.IsNullOrEmpty(Pattern))
						{
							foreach (Match MatchResult in Regex.Matches(Description, Pattern, RegexOptions.Multiline))
							{
								...
								{
									// 2022.10.27 强制使用第一个匹配到的工单地址。 如需修改可注释
									ClickHandler = () => SafeProcessStart(MatchResult.ToString());
			...
			if (Args.Button == MouseButtons.Left)
			{
				...
				if (HitTest.Item != null)
				{
					...
					if (Change != null)
					{
						...
						{
							LaunchEditor();
							Console.WriteLine("LaunchEditor....");
						}
						else
						{
		...
		{
			Console.WriteLine("in LaunchEditor = {0}");
			if (!Workspace.IsBusy() && Workspace.CurrentChangeNumber != -1)
			{
				...
				Console.WriteLine("EditorExe = {0}", EditorExe);
#if USECOUSTOMFUNCTION
...

				{
					...
					Console.WriteLine("LaunchArguments {0}", LaunchArguments.ToString());
				...
				else
				{
					if (MessageBox.Show("The editor needs to be built before you can run it. Build it now?", "Editor out of date", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
					{
						...
						Console.WriteLine("The editor needs to be built before you can r");
					}
				}
			}
		}
